...
struct threadmsg{
        ...
        long msgtype;
        /**
        * Holds the current queue lenght. Might not be meaningful if there's
        * several readers
        */
        long qlength;
 ...
 * You cannot call this if there are someone currently adding or getting
 * messages
 * from the queue.
 * After a queue have been cleaned, it cannot be used again untill
 * #thread_queue_init
 ...
 * messages
 * @return 0 on success EINVAL if queue is NULL EBUSY if someone is holding any
 * locks on the queue
 */
